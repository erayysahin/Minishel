...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/10/03 13:38:07 by kcevik            #+#    #+#             */
/*   Updated: 2024/10/03 16:06:12 by kcevik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	**args_and_path(t_data *data, t_token_node *node, char **path)
{
	char	**args;
	...
	else if (node && is_redir(node))
	{
		handle_redirection(node, data);
		while (node)
		{
			if (is_args(node) && node->prev && !is_redir(node->prev))
				break ;
			node = node->next;
		}
		args = get_command_args(node);
...
void	handle_child_process(t_data *data, t_token_node *node, int pipfd[2],
		char **env)
{
	char	**args;
	char	*path;
...
void	configure_fd(t_data *data, t_token_node *node, int in_out[2], int pipfd[2])
{
	if (data->in_fd != 0)
	{
		if (!in_out[0])
